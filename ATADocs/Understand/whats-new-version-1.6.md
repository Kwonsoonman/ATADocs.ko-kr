...
### 수동으로 이동된 데이터베이스에서 새 경로를 인식하지 못함
...
### ATA 1.5에서 업데이트할 때 마이그레이션 실패
ATA 1.6으로 업데이트할 때 다음 오류 코드로 인해 업데이트 프로세스가 실패할 수 있습니다.

![ATA 1.6 업데이트 오류](http://i.imgur.com/QrLSApr.png)이 오류가 표시되는 경우 **C:\사용자\<User>\AppData\Local\Temp**에서 배포 로그를 검토하고 다음 예외를 찾습니다.
...
### Net Framework 4.6.1에서 서버 다시 시작 요구
